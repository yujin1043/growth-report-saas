...
- 학생의 의도와 표현 방식 구체적으로 설명`
    ...
    if (progressStatus === 'started') {
      progressLabel = '오늘 처음 시작한 작품'
      progressTone = '새로운 시작의 설렘, 구상과 스케치 과정을 중심으로 묘사. 앞으로의 기대감을 담아 마무리.'
    } else if (progressStatus === 'none') {
      progressLabel = '진행 중인 작품'
      progressTone = '작업 과정, 기법 적용, 몰입하는 모습을 중심으로 묘사. 점점 완성되어가는 느낌을 담아 마무리.'
...
학부모님께 보내는 카카오톡 일일 수업 메시지를 작성합니다.

# 메시지 필수 구성 요소 (반드시 순서대로 포함)
1. 수업 주제 소개: 오늘 무엇을 그렸는지 자연스럽게 시작
2. 작품 설명: 주제를 어떻게 표현했는지 (구도, 분위기, 색감, 특징 등)
3. 미술 기법: 사용한 재료(${materials})에 맞는 기법 설명 (예: 번짐, 그라데이션, 붓터치, 선의 강약 등)
4. 오늘 진행 내용: 진행 상태에 맞는 구체적 작업 묘사와 마무리

# 현재 진행 상태: ${progressLabel}
톤 가이드: ${progressTone}
...
위 내용을 바탕으로 메시지를 작성해주세요.
반드시 수업 주제 → 작품 설명 → 미술 기법 → 진행 내용 순서로 포함해주세요.
